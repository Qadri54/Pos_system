<x-custom_component.management_layout>
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            <div>
                ...
                <p class="mb-0">Perbarui informasi outlet {{ $outlet->outlet_name }}</p>
            </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('outlets.update', $outlet->id) }}" method="POST" id="editOutletForm"
                        class="row g-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control @error('outlet_name') is-invalid @enderror"
                                id="outlet_name" name="outlet_name"
                        ...
                        <div class="col-12">
                            <label for="users" class="form-label fw-bold">
                                <i class="bi bi-people me-1"></i>
                                Kelola Staff Outlet
                            </label>
                            <select class="form-select @error('users') is-invalid @enderror" id="users" name="users[]"
                                multiple>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ $outlet->users->contains($user->id) ? 'selected' : '' }}>
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <h6 class="fw-bold mb-0">Informasi Saat Ini</h6>
                    </div>
                    <div class="mb-3">
                        <small class="text-muted d-block">ID Outlet</small>
                        <span class="badge bg-primary">#{{ $outlet->id }}</span>
                    </div>
                    <div class="mb-3">
                        <small class="text-muted d-block">Dibuat Pada</small>
                        <div class="fw-semibold">{{ $outlet->created_at->format('d M Y, H:i') }}</div>
                    </div>
                    <div class="mb-3">
                        <small class="text-muted d-block">Terakhir Diupdate</small>
                        <div class="fw-semibold">{{ $outlet->updated_at->format('d M Y, H:i') }}</div>
                    </div>
                    <div class="mb-0">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <h6 class="fw-bold mb-0">Staff Saat Ini ({{ $outlet->users->count() }})</h6>
                    </div>
                            ...
                            @foreach($outlet->users as $user)
                                <div class="d-flex align-items-center mb-2">
                                    <div class="rounded-circle p-2 me-3"
                                        style="background: linear-gradient(135deg, #4f46e5, #7c3aed); width: 35px; height: 35px; display: flex; align-items: center; justify-content: center;">
                    ...
                    @else
                        <div class="text-center text-muted py-3">
                            ...
                            <p class="mb-0 mt-2">Belum ada staff</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <h6 class="fw-bold mb-0">Statistik Outlet</h6>
                    </div>
                    <div class="row text-center">
                        <div class="col-6">
                            <div class="fw-bold text-primary fs-4">{{ $outlet->users->count() }}</div>
                            <small class="text-muted">Staff</small>
                        </div>
                        <div class="col-6">
                            <div class="fw-bold text-success fs-4">
                                {{ \App\Models\Order::where('outlet_id', $outlet->id)->count() }}
                            </div>
    ...
    <script>
        ...
        function previewData() {
            ...
            const selectedUsers = $('#users').select2('data');
        ...
        function submitForm() {
            if (validateForm()) {
                $('#editOutletForm')[0].submit();
            }
        }

        // Auto-save draft (optional feature)
        let autoSaveTimer;
        $('#outlet_name, #address').on('input', function () {
        ...
        function saveDraft() {
            ...
            localStorage.setItem('outlet_edit_draft_{{ $outlet->id }}', JSON.stringify(draftData));

            // Show save indicator
            const indicator = $('<small class="text-success ms-2"><i class="bi bi-check-circle me-1"></i>Draft tersimpan</small>');
            $('.card-header h5').append(indicator);
